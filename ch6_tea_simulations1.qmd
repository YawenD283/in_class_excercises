...
## Change the sample size from 48 to 100

```{r}
make_tea_data <- function(n_total = 100, sigma = 1.25, delta = 1.5) {
  n_half <- n_total / 2
...
# Change n_total from 18 and 48 to 36 and 96
...
tea_data <- make_tea_data(n_total = 36, delta=1.5)
```

```{r}
tea_data_highn <- make_tea_data(n_total = 96, delta=1.5)
```
...
By changing the sample size, the mean_rating of milk first among the tea data decreases from 5.11 to 5.00, while that of tea first increases from 3.78 to 3.94.
...
By changing the sample size, the mean_rating of milk first among the tea_highn data decreases from 5.04 to 4.67, while that of tea first increases from 3.46 to 3.56.
...
To do: Run these and look at the outputs by posting `out_low_n` in the console o
...
The p-value decreased from 0.057 (n_total = 18) to 0.012 (n_total = 36), indicating stronger evidence against the null hypothesis with the larger sample size. The confidence interval and degrees of freedom suggest more precision and accuracy in the estimates.

```{r}
out_high_n
```

Similarly, the p-value decreases, showing stronger evidence of differences provided by a larger sample size. Also, higher precision and accuracy are suggested.
...
## Change n_total from 18 to 36

```{r}
samps <- tibble(sim = 1:1000) |> # 
  mutate(data = map(sim, \(i) make_tea_data(n_total = 36, delta=1.5))) |>  # simulate
  unnest(cols = data) # wrangle
```

...where you have 48 participants per experiment

## Change n_total from 48 to 96

```{r}
samps_highn <- tibble(sim = 1:1000) |> # 
  mutate(data = map(sim, \(i) make_tea_data(n_total = 96, delta=1.5))) |>  # simulate
  unnest(cols = data) # wrangle
```
...
When the sample size increases, I do see some changes in the estimation of mean difference, but it is hard to tell the actual patterns of changing.
...
Comparing to the small sample size (n_total = 18), the larger sample size (n_total = 36) suggest decrease in variability in the estimate of mean difference. The estimates are more consistent across simulations.
...
When n_total = 96 (changed from n_total = 48), the estimates are more consistent, and should be closer to the true population mean difference.
...
null_model <- tibble(sim = 1:1000) |>
  mutate(data = map(sim, \(i) make_tea_data(n_total = 36, delta = 0))) |>
  unnest(cols = data)
...
When the sample size increases, the variability decreases.
...
When the sample size increases, the shuffled mean differences are more stable since the distributions are tighter.
...
When the sample size increases, the distribution of data points are relatively tighter.
...
```
When the sample size increases, the separation between the actual difference and the null distribution seems to have decreased. 
...
ggplot(data=tea_data_highn_shuffled_summary, aes(x=delta)) +
  ...
  theme_few()
```
When the sample size increases, the null distribution becomes narrower, and the null distribution stands out more clearly from the empirical distribution
...
```
When the sample size increased, the results appear to be more generalizable due to the increased precision and reduced variability.
...
```
When the sample size increases, the confidence interval becomes narrower and the mean ratings are more stabalized. 
...
```
I did not see any very significant difference. But when the sample size increases, there should be a narrower distribution and more clustering of p-values near 1.00
...
When the sample size increases, there appear to be a more polarized distribution, which means more p-values clustering around 0.
